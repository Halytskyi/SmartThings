...
// {"EEProm auto-push"}
const byte eepromTSensorAutoPushAddr[tSensorsNum] = {21, 22, 23, 24};
const DeviceAddress tSensorAddr[tSensorsNum] = {
  {0x28, 0xA0, 0x17, 0x16, 0xA8, 0x01, 0x3C, 0x54},
  {0x28, 0xAC, 0x21, 0x16, 0xA8, 0x01, 0x3C, 0x31},
  {0x28, 0xC1, 0xEB, 0x79, 0xA2, 0x00, 0x03, 0x7C},
  {0x28, 0x27, 0x0B, 0x79, 0xA2, 0x00, 0x03, 0x3D}};

// Temperature-humidity sensors DHT22
DHTNEW dht(17); // Temperature-humidity sensor pin A3
const byte dhtSensorsTypesNum = 2; // num sensors * num types
// {"command"}
const char *const dhtSensorCmd[dhtSensorsTypesNum] = {"O-t", "O-h"};
// {"auto-push"}
byte dhtSensorAutoPush[dhtSensorsTypesNum] = {0, 0};
// {"auto-push last update"}
unsigned long dhtSensorAutoPushLU[dhtSensorsTypesNum] = {0, 0};
// {"EEProm auto-push"}
const byte eepromDHTSensorAutoPushAddr[dhtSensorsTypesNum] = {25, 26};

// External alarm status
const byte alarmExternalPin = 4;
byte alarmExternalStatus = 0;
byte alarmExternalStatusFansDisabled = 0;
const byte eepromAlarmExternalStatus = 27;
unsigned long alarmExternalStatusAutoPushLastUpdate = 0;


float read_dht_sensor(const char sensor[]) {
  float readResult = 0.0;
  dht.read();
  if (strcmp(sensor, "O-t") == 0) {
    readResult = dht.getTemperature();
  } else if (strcmp(sensor, "O-h") == 0) {
    readResult = dht.getHumidity();
  }
  if (readResult < -30 or readResult > 150) {
    readResult = sqrt(-1);
  }
  return readResult;
}
...
void receiver_function(uint8_t *payload, uint16_t length, const PJON_Packet_Info &packet_info) {
  ...
  for (byte i = 0; i < dhtSensorsTypesNum; i += 1) {
    if (strcmp(command, dhtSensorCmd[i]) == 0) {
      if (valueLength == 0) {
        dtostrf(read_dht_sensor(dhtSensorCmd[i]), 0, 2, tmpBuf);
        busA_reply(request, tmpBuf);
        return;
      } else {
        busA_reply(request, badCommandReply);
    ...
    strcpy(tmpBuf, dhtSensorCmd[i]);
    strcat(tmpBuf, "-a");
    if (strcmp(command, tmpBuf) == 0) {
      if (valueLength == 0) {
        itoa(dhtSensorAutoPush[i], tmpBuf, 10);
        busA_reply(request, tmpBuf);
        return;
      } else {
        if (value_int == 0 or value_int == 1) {
          dhtSensorAutoPush[i] = value_int;
          EEPROM.update(eepromDHTSensorAutoPushAddr[i], value_int);
          busA_reply(request, goodCommandReply);
          return;
        } else {
          busA_reply(request, badCommandReply);
...
void autopush() {
  ...
  if (curMillis - prevMillisAutoPush >= minAutoPushInterval) {
    ...
    if (msgPushed == 0) {
      for (byte i = 0; i < dhtSensorsTypesNum; i += 1) {
        if (dhtSensorAutoPush[i] == 1) {
          if (curMillis - dhtSensorAutoPushLU[i] >= autoPushInterval and curMillis - lastUpdateMillisAutoPush >= minAutoPushInterval) {
            char tmpBuf[8];
            dtostrf(read_dht_sensor(dhtSensorCmd[i]), 0, 2, tmpBuf);
            busB_send(dhtSensorCmd[i], tmpBuf);
            dhtSensorAutoPushLU[i] = curMillis;
            lastUpdateMillisAutoPush = curMillis;
            msgPushed = 1;
          }
        }
...
void setup() {
  if (EEPROM.read(ADDR_VERSION) != CURRENT_EEPROM_VERSION) { //EEprom is wrong version or was not programmed, write default values to the EEprom
    ...
    for (byte i = 0; i < dhtSensorsTypesNum; i += 1) {
      EEPROM.update(eepromDHTSensorAutoPushAddr[i], dhtSensorAutoPush[i]);
    }
    EEPROM.update(eepromAlarmExternalStatus, alarmExternalStatus);
    ...
    for (byte i = 0; i < dhtSensorsTypesNum; i += 1) {
      dhtSensorAutoPush[i] = EEPROM.read(eepromDHTSensorAutoPushAddr[i]);
    }
